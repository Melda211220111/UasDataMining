...
# Footer Insightsimport streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
...
with col3:
    ...
    drug_count = data_filtered['Drug'].value_counts()
    fig, ax = plt.subplots()
    ax.bar(drug_count.index, drug_count.values, color='skyblue')
    ax.set_title("Drug Distribution", fontsize=14, fontweight='bold')
    ax.set_xlabel("Drug Type", fontsize=12)
    ax.set_ylabel("Count", fontsize=12)
    for i, val in enumerate(drug_count.values):
...
with col4:
    st.markdown("#### Na_to_K vs Age")
    fig, ax = plt.subplots()
    ax.scatter(data_filtered['Age'], data_filtered['Na_to_K'], c='blue', alpha=0.6, edgecolor='black')
    ax.set_title("Na_to_K vs Age", fontsize=14, fontweight='bold')
    ax.set_xlabel("Age", fontsize=12)
    ax.set_ylabel("Na_to_K", fontsize=12)
    st.pyplot(fig)
...
with col5:
    st.markdown("#### Blood Pressure and Cholesterol Levels")
    fig, ax = plt.subplots()
    bp_cholesterol = data_filtered.groupby(['BP', 'Cholesterol']).size().unstack(fill_value=0)
    bp_cholesterol.plot(kind='bar', stacked=True, ax=ax, color=['green', 'red'], edgecolor='black')
    ax.set_title("BP and Cholesterol Distribution", fontsize=14, fontweight='bold')
    ax.set_xlabel("Blood Pressure Levels", fontsize=12)
    ax.set_ylabel("Count", fontsize=12)
    st.pyplot(fig)
